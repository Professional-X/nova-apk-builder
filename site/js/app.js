...
async function pollBuildStatus() {
  ...
  try {
    ...
    if (!run) {
      updateStep(1, 'active', 'Waiting for build to start...');
    ...
    if (status === 'queued') {
      ...
      updateStep(1, 'active', 'Waiting for GitHub Actions runner...');
    } else if (status === 'in_progress') {
      setProgressTitle('Building APK...');
      updateStep(1, 'done', 'Build environment ready');
      updateStep(2, 'active', 'Compiling your project...');
      updateStep(3, 'active', 'Signing APK...');
    }

    if (status === 'completed') {
      if (conclusion === 'success') {
        updateStep(1, 'done', 'Build environment ready');
        updateStep(2, 'done', 'Project compiled successfully');
        updateStep(3, 'done', 'APK signed');
        updateStep(4, 'active', 'Preparing download links...');
...
async function onBuildFailure(run) {
  updateStep(1, 'error', 'Build failed');
